...
partical sum[32]就是由Y的最高3bit选择出来的！而选择出来的X，只能是+0，或者+X，不会出现-X和-2X的情况！！这就意味着**partical sum[32]的low bit carray一定是0，可以直接丢弃。** 本项目中采用的即为最后一种方法。

因此，方法3结合了方法1和方法2的全部优点，在不增加华莱士树输入的前提下，通过partial sum直接的合并，去掉了partial sum的低bit+1延迟。
